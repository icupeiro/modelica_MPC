...
package Case900GEOTABS
  model InternalUpdate
    ...
    Modelica.Blocks.Sources.RealExpression optVar1(y=mpcCase900GEOTABS_1bor.yOpt[
  ...
  public
    Modelica.Blocks.Sources.RealExpression optVar3(y=mpcCase900GEOTABS_1bor.yOpt[
          3])
  ...
  Modelica.Blocks.Sources.Constant const(k=273.15 + 10.8)
    ...
    Modelica.Blocks.Sources.RealExpression slack(y=if rectangularZoneTemplate.TAir
           >= 295 then 0 else 295 - rectangularZoneTemplate.TAir)
      annotation (Placement(transformation(extent={{-90,36},{-70,56}})));
    Modelica.Blocks.Continuous.Integrator energy
      annotation (Placement(transformation(extent={{106,-24},{126,-4}})));
    Modelica.Blocks.Continuous.Integrator discomfort
  ...
  initial equation
    r[1] = r_a;
    r[2] = r_a + sqrt(borFieDat.soiDat.kSoi/borFieDat.soiDat.cSoi/borFieDat.soiDat.dSoi*60) "eskilson minimum length";
    r[3] = r_a + 2*sqrt(borFieDat.soiDat.kSoi/borFieDat.soiDat.cSoi/borFieDat.soiDat.dSoi*60);
    r[4] = r_a + 3*sqrt(borFieDat.soiDat.kSoi/borFieDat.soiDat.cSoi/borFieDat.soiDat.dSoi*60);
  ...
  connect(const.y, borefieldOneUTube_r.TSoi) annotation (Line(points={{65.4,
          40},{58,40},{58,16},{54,16}}, color={0,0,127}));

    connect(embeddedPipe.heatPortEmb[1], boundaryWall.port_emb[1]) annotation (
        Line(points={{26,-50.6667},{26,-38},{2,-38}}, color={191,0,0}));
    connect(embeddedPipe.heatPortEmb[2], boundaryWall.port_emb[1])
      annotation (Line(points={{26,-50},{26,-38},{2,-38}}, color={191,0,0}));
    connect(embeddedPipe.heatPortEmb[3], boundaryWall.port_emb[1]) annotation (
        Line(points={{26,-49.3333},{26,-38},{2,-38}}, color={191,0,0}));
    connect(heatPump.W_comp, energy.u) annotation (Line(points={{81,-54},{94,
            -54},{94,-14},{104,-14}}, color={0,0,127}));
    connect(slack.y, discomfort.u) annotation (Line(points={{-69,46},{18,46},{
            18,18},{104,18}}, color={0,0,127}));
...
First implementation
  ...
  Modelica.Blocks.Sources.Constant const(k=273.15 + 10.8)
    annotation (Placement(transformation(extent={{78,34},{66,46}})));
  public
    Modelica.Blocks.Sources.RealExpression[10] TGround(y=borefieldOneUTube_r.TGround[1:10])
      "temperatures on the RC network"
      annotation (Placement(transformation(extent={{-100,36},{-80,56}})));
    Modelica.Blocks.Sources.RealExpression TGroundExt(y=borefieldOneUTube_r.TGround[11]) "temperature of the external boundary"
