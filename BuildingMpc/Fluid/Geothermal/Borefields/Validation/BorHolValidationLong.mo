...
model BorHolValidationLong
  ...
  IBPSA.Fluid.Sources.MassFlowSource_T sou1(
    ...
    use_T_in=false,
    m_flow=borFieDat.conDat.mBor_flow_nominal,
    T=277.15) "Source" annotation (Placement(transformation(extent={{-76,-70},{-56,
            -50}},    rotation=0)));
  ...
  tLoaAgg=tLoaAgg,
  p_max=p_max)                              "Ground temperature response"
...
equation
  ...
  connect(TBorDisOut.T, add.u1)
    annotation (Line(points={{40,11},{40,54},{58,54}}, color={0,0,127}));
  connect(TBorConOut.T, add.u2) annotation (Line(points={{40,-49},{48,-49},{48,42},
          {58,42}}, color={0,0,127}));
  connect(groTemRes.borWall, therCol.port_b)
    annotation (Line(points={{-40,70},{-30,70}}, color={191,0,0}));
  connect(groTemRes.TSoi, TGroUn.y)
